...
*/
/*
 * Диагностический вывод результатов проверки пары соседних GPIO.
 *
 * Формат:
 *   GPIO XX connected to YY C[AB->DE]
 *
 * где:
 *   XX  - номер первого пина;
 *   YY  - номер второго пина;
 *
 *   C   - признак специального случая (бит 5 результата testPins()).
 *         Обычно означает, что соединение было подтверждено
 *         активным тестом, а не только анализом подтяжек.
 *
 *   A   - логический уровень на pin0 при включённой внутренней pull-down.
 *   B   - логический уровень на pin0 при включённой внутренней pull-up.
 *   D   - логический уровень на pin1 при включённой внутренней pull-down.
 *   E   - логический уровень на pin1 при включённой внутренней pull-up.
 *
 * Значения:
 *   0 - вход удалось притянуть к нулю.
 *   1 - вход остаётся в единице.
 *
 * По комбинациям AB и DE можно определить характер внешней обвязки:
 *
 *   00 - линия жёстко удерживается в нуле
 *        (внешняя подтяжка к GND или активный драйвер).
 *
 *   01 - линия "плавающая"
 *        (внутренние подтяжки работают как ожидается).
 *
 *   11 - линия жёстко удерживается в единице
 *        (внешняя подтяжка к VCC или активный драйвер).
 *
 * Если установлен бит connect (бит 0 результата testPins()),
 * выводится строка "connected", иначе "not connected".
 *
 * Пример:
 *   GPIO 06 connected to 07 1[01->01]
 *
 * означает:
 *   - между GPIO6 и GPIO7 обнаружена связь;
 *   - оба вывода выглядят как неподтянутые (01);
 *   - связь подтверждена активным тестом (C=1).
 
0[00->00]  обе линии удерживаются в 0.
            Возможна связь. Выполняется активная проверка.
            При успехе станет 1[00->00].

0[00->01]  pin0 удержан в 0, pin1 плавающий.
            Связь считается невозможной.

0[00->10]  pin0 удержан в 0, pin1 даёт противоречивый результат.
            Связь считается невозможной.

0[00->11]  pin0 удержан в 0, pin1 удержан в 1.
            Связь считается невозможной.

0[01->00]  pin0 плавающий, pin1 удержан в 0.
            Связь считается невозможной.

0[01->01]  обе линии плавающие (обычный GPIO).
            Возможна связь. Выполняется активная проверка.
            При успехе станет 1[01->01].

0[01->10]  pin0 плавающий, pin1 противоречивый.
            Связь считается невозможной.

0[01->11]  pin0 плавающий, pin1 удержан в 1.
            Связь считается невозможной.

0[10->00]  pin0 противоречивый, pin1 удержан в 0.
            Связь считается невозможной.

0[10->01]  pin0 противоречивый, pin1 плавающий.
            Связь считается невозможной.

1[10->10]  специальный случай.
            Код считает связь существующей без дополнительной проверки.
            В комментарии отмечено как сомнительный режим.

0[10->11]  pin0 противоречивый, pin1 удержан в 1.
            Связь считается невозможной.

0[11->00]  pin0 удержан в 1, pin1 удержан в 0.
            Связь считается невозможной.

0[11->01]  pin0 удержан в 1, pin1 плавающий.
            Связь считается невозможной.

0[11->10]  pin0 удержан в 1, pin1 противоречивый.
            Связь считается невозможной.

0[11->11]  обе линии удерживаются в 1.
            Возможна связь. Выполняется активная проверка.
            При успехе станет 1[11->11]. 
